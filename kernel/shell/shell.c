...
void shell_init(void) {
	...
	cli_register("cd", "change the current directory", cmd_cd);
	cli_register("ls", "list directory contents", cmd_ls);
	cli_register("clear", "clear the console", cmd_clear);
	cli_register("touch", "create an empty file", cmd_touch);
	cli_register("mkdir", "create a directory", cmd_mkdir);
	cli_register("rm", "remove a file", cmd_rm);
	cli_register("rmdir", "remove an empty directory", cmd_rmdir);
	cli_register("cat", "print a file", cmd_cat);
...
static bool shell_add_history(const char *input) {
	...
	if (shell_context.history_count == shell_context.history_capacity) {
		...
		new_history = krealloc(shell_context.history,
...
void shell_run(tty_t *tty) {
    ...
    printf("\033[15,0]Welcome to the shell ... Type 'help' for a list of commands.\n");
	while (1) {
