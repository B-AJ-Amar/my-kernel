...
static void bitmap_set_range(uint32_t start_page, uint32_t num_pages,bool is_reserved);
...
static void mem_set_range(uint64_t base, uint64_t length,bool is_reserved)
{
  if (length == 0 || base >= MAX_PHYS_MEMORY || base + length > MAX_PHYS_MEMORY) return;
