...
void set_idt_gate(uint8_t num, uint32_t interupt_addr, uint16_t selector,
                  uint8_t flags) {
  idt[num].offset_low = interupt_addr & 0xFFFF;
  idt[num].selector = selector;
  idt[num].zero = 0;
  idt[num].flags = flags;
  idt[num].offset_high = (interupt_addr >> 16) & 0xFFFF;
}
