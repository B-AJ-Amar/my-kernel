...
void pic_send_command(uint8_t pic, uint8_t cmd)
{
    ...
    else panic("Invalid PIC number: %u\n", pic);
}
...
void pic_init(void){
    ...
    pic_send_data(PIC2, 0x02); // tell Slave PIC connected to the master via the second pin (00000010)  (if its connected to the third => 00000011)
