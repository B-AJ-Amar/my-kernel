...
bool fs_create(fs_node_t *parent, fs_node_type_t type, const char *name) {
    ...
    if (parent->children.count == parent->children.capacity) {
        ...
        fs_node_t **new_items = (fs_node_t **)krealloc(parent->children.items, new_capacity * sizeof(fs_node_t *));
        if (!new_items) {
...
fs_base_node_t *fs_list(fs_node_t *parent) {
    ...
    fs_base_node_t *list = (fs_base_node_t *)kmalloc((parent->children.count + 1) * sizeof( fs_base_node_t));
    if (!list) {
    ...
    for (size_t i = 0; i < parent->children.count; i++) {
        ...
        snprintf(list[i].name, sizeof(list[i].name), "%s", parent->children.items[i]->name);
    }
